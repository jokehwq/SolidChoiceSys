...
<body>
		...
		<li class="active"><a href="#">默认回复<shiro:hasPermission name="reply:wxSubscribeMsg:edit">${not empty wxSubscribeMsg.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="reply:wxSubscribeMsg:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="wxSecondSubscribeMsg" action="${ctx}/wx_second_subscribe/wxSecondSubscribeMsg/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			...
			<div class="controls">
				<input type="hidden" name="keywordId" id='accountid' value="${accountid}"/>
			</div>
			<button class='chooseMsgType' id='chooseMsgType' type="button" >选择消息类型</button>
			<div class="controls">
				<input type="hidden" name="id"  value="${wxKeywordMsg.id}"/>
			</div>
			<div class="controls">
				<input type="hidden" name="msgId" id="msgContentId" />
			</div>
		</div>
		<div class="control-group">
		</div>
		...
		<div class="form-actions">
			<shiro:hasPermission name="wx_second_subscribe:wxSecondSubscribeMsg:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>

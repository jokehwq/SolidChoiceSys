...
<ul class="nav nav-tabs">
  <li><a href="${ctx}/sys/voting/">字典信息列表</a></li>
  <li class="active"><a href="${ctx}/sys/voting/form?id=${voting.id}">字典信息<shiro:hasPermission
          name="sys:voting:edit">${not empty voting.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission
          name="sys:voting:edit">查看</shiro:lacksPermission></a></li>
</ul>
<br/>
<form:form id="inputForm" modelAttribute="voting" action="${ctx}/sys/voting/save" method="post"
  ...
  <div class="control-group">
    ...
    <div class="col-sm-9 big-photo">
      <div id="preview">
        <c:choose>
          <c:when test="${voting.id == '' || voting.id == null}">
            <c:if test="${voting.img_url != null}" >
          ...
          <c:when test="${voting.id != '' || voting.id != null}">
            <c:if test="${voting.img_url == '' || voting.img_url == null}" >
              <img  style="margin-left: 20px;width: 70px;" id="imghead" border="0" src="${ctxStatic}/images/photo_icon.png" onclick="$('#previewImg').click()" width="300px"/>
            </c:if>
            <c:if test="${voting.img_url != null}" >
              <img   style="margin-left: 20px;width: 70px;" id="imghead" border="0" src="${voting.img_url}" onclick="$('#previewImg').click()">
            </c:if>
          </c:when>
      ...
      <input id="filepath" type="hidden" name="url" value="${voting.id}" accept="image/gif,image/jpeg">
  ...
  <div class="form-actions">
    <shiro:hasPermission name="sys:voting:edit"><input id="btnSubmit" class="btn btn-primary" type="submit"
                                                       value="保 存"/>&nbsp;</shiro:hasPermission>
    <input id="btnCancel" class="btn" type="button" value="返 回"
           onclick="window.location.href='${ctx}/sys/voting'"/>
  </div>
</form:form>
</body>
</html>
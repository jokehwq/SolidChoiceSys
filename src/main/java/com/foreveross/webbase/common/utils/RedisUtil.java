...
public class RedisUtil {
    ...
    // Redis服务器IP
    private static String IP = Global.getConfig("redis.host");

    // Redis的端口号
    private static int PORT =  Integer.parseInt(Global.getConfig("redis.port"));

    // 访问密码
    private static String PASSWORD = Global.getConfig("redis.password");
    // 可用连接实例的最大数目，默认值为8；
    // 如果赋值为-1，则表示不限制；如果pool已经分配了maxActive个jedis实例，则此时pool的状态为exhausted(耗尽)。
    private static int MAX_ACTIVE =Integer.parseInt(Global.getConfig("redis.max_active"));

    // 控制一个pool最多有多少个状态为idle(空闲的)的jedis实例，默认值也是8。
    private static int MAX_IDLE = Integer.parseInt(Global.getConfig("redis.max_idle"));

    // 等待可用连接的最大时间，单位毫秒，默认值为-1，表示永不超时。如果超过等待时间，则直接抛出JedisConnectionException；
    private static int MAX_WAIT = Integer.parseInt(Global.getConfig("redis.max_wait"));
     ...
     * @param value
     */
    public synchronized static void setSet(String key, String value) {
        try {
            value = StringUtils.isBlank(value) ? "" : value;
            Jedis jedis = getJedis();
     ...
     * @param seconds 以秒为单位
     */
    public synchronized static void setSet(String key, String value, int seconds) {
        try {
            value = StringUtils.isBlank(value) ? "" : value;
            Jedis jedis = getJedis();
            jedis.sadd(key,value);
     ...
     * @param key
     */
    public synchronized static void removeSetValue(String key,String... value) {
        try {
            Jedis jedis = getJedis();

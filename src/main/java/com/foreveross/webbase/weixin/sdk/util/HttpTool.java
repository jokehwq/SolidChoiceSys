...
public class HttpTool {
	...
	public static String get(String url) {
		if (log.isDebugEnabled()) {
			log.debugf("Request url: %s, default timeout: %d", url,
					CONNECT_TIME_OUT);
		}
		...
		try {
			...
			if (resp.isOK()) {
				...
				if (log.isInfoEnabled()) {
					log.infof("GET Request success. Response content: %s",
							content);
				}
				return content;
			}

			throw Lang.wrapThrow(new RuntimeException(String.format(
					"Get request [%s] failed. status: %d", url,
					resp.getStatus())));
		} catch (Exception e) {
	...
	public static String post(String url, String body,String charset) {
		...
		try {
			...
			if (resp.isOK()) {
				if(null==charset){
					 content = resp.getContent(null);
				}else{
					 content = resp.getContent(charset);
				}
				
				if (log.isInfoEnabled()) {
					log.infof("POST Request success. Response content: %s",
							content);
				}
				return content;
			}
			...
			throw Lang.wrapThrow(new RuntimeException(String.format(
					"Post request [%s] failed. status: %d", url,
					resp.getStatus())));
		} catch (Exception e) {
			throw Lang.wrapThrow(e);
		}
	...
	public static String upload(String url, File file) {
		if (log.isDebugEnabled()) {
			log.debugf("Upload url: %s, file name: %s, default timeout: %d",
					url, file.getName(), CONNECT_TIME_OUT);
		}
		...
		try {
			...
			throw Lang.wrapThrow(new RuntimeException(String.format(
					"Upload file [%s] failed. status: %d", url,
					resp.getStatus())));
		} catch (Exception e) {
			throw Lang.wrapThrow(e);
		}
	...
	public static Object download(String url) {
		if (log.isDebugEnabled()) {
			log.debugf("Download url: %s, default timeout: %d", url,
					CONNECT_TIME_OUT);
		}
		...
		try {
			...
			if (resp.isOK()) {
				...
				cd = cd.substring(cd.indexOf(FILE_NAME_FLAG)
						+ FILE_NAME_FLAG.length());
			...
			throw Lang.wrapThrow(new RuntimeException(String.format(
					"Download file [%s] failed. status: %d, content: %s", url,
					resp.getStatus(), resp.getContent())));
		} catch (Exception e) {
			throw Lang.wrapThrow(e);
		}
	}
